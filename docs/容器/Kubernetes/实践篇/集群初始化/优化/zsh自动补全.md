...
[官方文档](https://kubernetes.io/zh/docs/tasks/tools/included/optional-kubectl-configs-zsh/)

首先通过 `$ kubectl completion zsh` 命令得到要 `zsh` 配置信息:

<details>
<summary>展开查看命令执行结果</summary>
...
接下来拷贝命令执行结果，写入到 `~/.zshrc` 文件最后，并通过命令 `$ source ~/.zshrc` 使得配置生效即可。
